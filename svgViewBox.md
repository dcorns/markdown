...
When we adjust our viewBox width and height proportionately, we can also count on the boundaries of virtual XY as viewable within our viewport to be consistent. ```Xmin = -(Wvb/2), Xmax = Wvb/2, Ymin = -(Hvb/2), Ymax = Hvb/2```. However, when we change the values of Wvb and Hvb independently of each other, the rules governing the bounds of our virtual space become a little more complex. We will, instead, need to apply the following rules to obtain the boundary limitations of our viewBox.
 ...
 Knowing this, we may choose to simply change the value of Hvb or Wvb alone when zooming in and out of virtual space. The Following illustrations show the results of using just Wvb values to zoom in and out of virtual space. Note that because we do not change Hvb, we do not need to recalculate the min-y values to maintain our center reference.
