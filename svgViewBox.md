...
The viewBox is an attribute that we can add to our SVG to do some interesting things. A viewBox attribute has four properties that can be separated by commas or spaces. They are min-x, min-y, width, and height. Lets explore how these settings effect our svg first by setting the viewBox height and width the same as the SVG height and width and while leaving them at that setting, varying the viewBox values for min-x and min-y. If we use the settings ```viewBox="0 0 svg-width svg-height``` we will observe that the view box attribute has no effect on the way our svg looks. However, if we we change the min-x and min-y values, something interesting occurs. Lets say we had the following svg defined:
...
You may want to drop this code into [jsBin](https://jsbin.com/yoyip/4/edit?html,output) and add the following css in order to visualize what is going on.
 ...
 Referring back to our JSBin example, what if we wanted to expand our window to include both of our circles. Intuitively, we might think that we would merely need to increase the width and height of our viewBox to give us a larger window. Set min-y and min-x back to 0. Then set the viewBox height to 300. This will exposes the entire lower circle and also leave the upper circle exposed. But what happened to the position of the upper circle? We only changed the viewBox height and the upper circle moved to the right. That's not what we want. We did not get a larger window. Of course you knew it would not be that simple. So if we did not make the window larger, what did we do? We added more virtual space inside our window or putting it another way, we zoomed out. Ok, that is clear enough, but we only changed the viewBox height. Why did the virtual space expand horizontally as well as vertically? It will be easier see what happens if we add a rectangle with the same dimensions as our SVG. Add the the following code as the last line inside of our SVG.
