...
When we adjust our viewBox width and height proportionately, we can also count on the boundaries of virtual XY that is viewable within our viewport to also be consistent. ```-Xmax = min-x, +Xmax = -(min-x), -Ymax = min-y, +Ymax = -(min-y)```. However, when we change the values of Wvb and Hvb independently of each other, the rules governing the bounds of our virtual space become a little more complex. We will, instead need to apply these rules to obtain the boundary limitations of our viewBox. If we increase the height of the view box alone, then the boundaries of the height are calculated as normal, but the boundaries of the width become ```-Xmax = Wvb + ((Wvb - Hvb)/2)```. If we increase only the width of our viewBox then the boundaries of the width are calculated as normal, but the boundaries of the height become ```((Hvb - Wvb)/2)```. 
